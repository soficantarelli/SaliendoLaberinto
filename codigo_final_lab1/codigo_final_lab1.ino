...
void motorTurn(int direction, int time) {motor1.run(RELEASE);
  motor2.run(RELEASE);
  delay(500);
  switch(direction) {
    case 1:
      motor1.setSpeed(120 + Kp*direction); 
      motor2.setSpeed(130 + Kp*direction);
      motor1.run(FORWARD);
      motor2.run(BACKWARD);
      break;
    case 2:
    ...
    case 3:
      ...
      break;
    case -1:
      motor1.setSpeed(130 + Kp*(-direction)); 
...
void optimizado(int direction, int degrees) {
  motorForward();
  delay(1500);
  motorTurn(direction, degrees);  
}
...
void cambiarDireccion(char dir) {
  switch(dir) {
    case 'L':
      optimizado(IZQUIERDA, 1350);           
      break;   
    case 'R': 
     optimizado(DERECHA, 1350);     
      break;   
    case 'B':
      optimizado(IZQUIERDA, 2600);     
      break;   
    case 'S': 
      runExtraInch();
      break;
